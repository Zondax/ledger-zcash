...
#[inline(never)]
pub fn derive_zip32_child_fromseedandpath_ida(seed: &[u8; 32], path: &[u32]) -> [u8; 160] {
    //ASSERT: len(path) == len(harden)
    ...
    let nk_tmp = constants::PROVING_KEY_BASE.multiply_bits(&nsk.to_bytes());
    let nk = AffinePoint::from(nk_tmp);//.to_bytes();


    let mut result = [0u8; 160];
    // ak.copy_from_slice(&point);
    result[0..32].copy_from_slice(&divkey);
    result[32..64].copy_from_slice(&ask.to_bytes());
    result[64..96].copy_from_slice(&nsk.to_bytes());
...
#[no_mangle]
pub extern "C" fn zip32_child_ida(
    seed_ptr: *const [u8; 32],
    dk_ptr: *mut [u8; 32],
    ak_ptr: *mut [u8; 32],
    nk_ptr: *mut [u8; 32],
    ...
    const FIRSTVALUE: u32 = 32 ^ 0x8000_0000;
    const COIN_TYPE: u32 = 133 ^ 0x8000_0000; //hardened, fixed value from https://github.com/adityapk00/librustzcash/blob/master/zcash_client_backend/src/constants/mainnet.rs
    let k = derive_zip32_child_fromseedandpath_ida(seed, &[FIRSTVALUE, COIN_TYPE, pos]); //consistent with zecwallet

    // k = dk || ask || nsk || ak || nk
    dk.copy_from_slice(&k[0..32]);
    ak.copy_from_slice(&k[96..128]);
    nk.copy_from_slice(&k[128..160]);
}
...
#[no_mangle]
pub extern "C" fn zip32_child_ask_nsk_ida(
    seed_ptr: *const [u8; 32],
    ask_ptr: *mut [u8; 32],
    nsk_ptr: *mut [u8; 32],
    pos: u32,
...
mod tests {
    ...
    #[test]
    fn test_zip32_childaddress_ida() {
        let seed = [0u8; 32];

        let p: u32 = 0x8000_0001;
        let keys = derive_zip32_child_fromseedandpath_ida(&seed, &[p]);
        ...
        let mut ak_ida = [0u8; 32];
        ak_ida.copy_from_slice(&keys[96..128]);

        let mut nk_ida = [0u8; 32];
        nk_ida.copy_from_slice(&keys[128..160]);
    ...
    #[test]
    fn test_zip32_childaddress_ledgerkey_ida() {
        //e91db3f6c120a86ece0de8d21d452dcdcb708d563494e60a6cee676f5047ded7
        let s = hex::decode("b08e3d98da431cef4566a13c1bb348b982f7d8e743b43bb62557ba51994b1257")
            .expect("error");
        let seed: [u8; 32] = s.as_slice().try_into().expect("er");
        ...
        let p: u32 = 1000 | 0x8000_0000;
        let keys = derive_zip32_child_fromseedandpath_ida(&seed, &[FIRSTVALUE, COIN_TYPE, p]);
        ...
        let mut ak = [0u8; 32];
        ak.copy_from_slice(&keys[96..128]);

        let mut nk = [0u8; 32];
        nk.copy_from_slice(&keys[128..160]);
