...
// FIXME: Move RedJubJub to another file
// #[no_mangle]
// pub extern "C" fn redjubjub_generate(result: *mut [u8; 32]) -> u32 {
//     // Generating RedJubjub private key
//     let mut rng = bolos::Trng {};
//     let sk = redjubjub::PrivateKey::generate(&mut rng);
//
//     let result = unsafe { &mut *result };
//     result.copy_from_slice(&sk.to_bytes());
//
//     32
// }
//
// #[no_mangle]
// pub extern "C" fn redjubjub_to_public(sk: *const [u8; 32], result: *mut [u8; 32]) -> u32 {
//     // Computing RedJubjub public key from private key
//     let sk = redjubjub::PrivateKey::from_bytes(unsafe { &(*sk) });
//     if sk.is_none().into() {
//         // Invalid PrivateKey!
//         return 0;
//     }
//     let sk = sk.unwrap();
//
//     let vk = redjubjub::PublicKey::from_private(&sk, &constants::SPENDING_KEY_BASE);
//
//     let result = unsafe { &mut *result };
//     result.copy_from_slice(&vk.to_bytes());
//
//     32
// }
//
// FIXME: Move Zip32 to another file
// #[no_mangle]
// pub extern "C" fn zip32address_master(result: *mut [u8; 43]) -> u32 { //fixme
//     let seed: [u8; 32] = [0; 32]; //fix with seed from Ledger
//     return 43;
// }
//
// //m_Sapling / purpose' / coin_type' / account' -> parsing done before this
// //account = child number in [0,2^31 - 1]
// #[no_mangle]
// pub extern "C" fn zip32address_fromaccount(account: u32, result: *mut [u8; 43]) -> u32 { //fixme
//     let seed: [u8; 32] = [0; 32]; //fix with seed from Ledger
//     let result = unsafe { &mut *result };
//
//     return 43;
// }
//
// //m_Sapling / purpose' / coin_type' / account' / address_index -> parsing should be done before this
// //returns first valid address starting at address_index (should it return the first valid diversifier index as well)?
// //account = child number in [0,2^31 - 1]
// //address_index = diversifier
// #[no_mangle]
// pub extern "C" fn zip32address_fromaccount_withindex(account: u32, address_index: [u8; 11], result: *mut [u8; 43]) -> u32 { //fixme
//     let seed: [u8; 32] = [0; 32]; //fix with seed from Ledger
//     43
// }
