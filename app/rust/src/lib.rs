...
fn kdf_sapling(dhsecret: [u8; 32], epk: [u8; 32]) -> [u8; 64] {
...
fn chacha_encryptnote(key: [u8;32], plaintext: [u8;NOTE_PLAINTEXT_SIZE]) -> [u8;ENC_CIPHERTEXT_SIZE]{
    let mut output = [0u8; ENC_CIPHERTEXT_SIZE];
        ChachaPolyIetf::aead_cipher()
            .seal_to(&mut output, &plaintext, &[], &key, &[0u8; 12]).unwrap();
    output
}

fn chacha_decryptnote(key: [u8;32], ciphertext: [u8;ENC_CIPHERTEXT_SIZE]) -> [u8;NOTE_PLAINTEXT_SIZE]{
...
mod tests {
    ...
    fn test_enc(){
        let x : [u8;32] = [
            0x81, 0xc7, 0xb2, 0x17, 0x1f, 0xf4, 0x41, 0x52,
            0x50, 0xca, 0xc0, 0x1f, 0x59, 0x82, 0xfd, 0x8f,
            0x49, 0x61, 0x9d, 0x61, 0xad, 0x78, 0xf6, 0x83,
            0x0b, 0x3c, 0x60, 0x61, 0x45, 0x96, 0x2a, 0x0e
        ];
        let y : [u8;32] = [
            0x88, 0x99, 0xc6, 0x44, 0xbf, 0xc6, 0x0f, 0x87,
            0x83, 0xf9, 0x2b, 0xa9, 0xf8, 0x18, 0x9e, 0xd2,
            0x77, 0xbf, 0x68, 0x3d, 0x5d, 0x1d, 0xae, 0x02,
            0xc5, 0x71, 0xff, 0x47, 0x86, 0x9a, 0x0b, 0xa6];
        let z : [u8;32] = [
            0x2e, 0x35, 0x7d, 0x82, 0x2e, 0x02, 0xdc, 0xe8,
            0x84, 0xee, 0x94, 0x8a, 0xb4, 0xff, 0xb3, 0x20,
            0x6b, 0xa5, 0x74, 0x77, 0xac, 0x7d, 0x7b, 0x07,
            0xed, 0x44, 0x6c, 0x3b, 0xe4, 0x48, 0x1b, 0x3e
        ];
        assert_eq!(sapling_ka_agree(x,y),z);
