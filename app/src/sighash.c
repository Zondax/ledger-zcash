...
// #define  ZCASH_JOINSPLITS_HASH_PERSONALIZATION "ZcashJSplitsHash" not
// supported
...
// const uint8_t CONSENSUS_BRANCH_ID_SAPLING[4] = {0x76, 0xb8, 0x09, 0xBB}; //
// sapling const uint8_t CONSENSUS_BRANCH_ID_ORCHARD[4] = {0xC2, 0xD6, 0xD0,
// 0xB4};       // orchard
//
const uint8_t CONSENSUS_BRANCH_ID_SAPLING[4] = {0xBB, 0x09, 0xB8,
                                                0x76}; // sapling
const uint8_t CONSENSUS_BRANCH_ID_ORCHARD[4] = {0xB4, 0xD0, 0xD6,
                                                0xC2}; // orchard

void sapling_transparent_prevouts_hash(const uint8_t *input, uint8_t *output) {
  const uint8_t n = t_inlist_len();
  cx_blake2b_t ctx;
  cx_blake2b_init2_no_throw(&ctx, 256, NULL, 0,
                            (uint8_t *)ZCASH_PREVOUTS_HASH_PERSONALIZATION, 16);
...
void sapling_transparent_sequence_hash(const uint8_t *input, uint8_t *output) {
  ...
  cx_blake2b_t ctx;
  cx_blake2b_init2_no_throw(&ctx, 256, NULL, 0,
                            (uint8_t *)ZCASH_SEQUENCE_HASH_PERSONALIZATION, 16);
...
void v4_transparent_outputs_hash(uint8_t *output) {
  ...
  cx_blake2b_t ctx;
  cx_blake2b_init2_no_throw(&ctx, 256, NULL, 0,
                            (uint8_t *)ZCASH_OUTPUTS_HASH_PERSONALIZATION, 16);
...
void shielded_output_hash(const uint8_t *input, uint16_t inputlen,
  ...
  cx_blake2b_init2_no_throw(
      &ctx, 256, NULL, 0,
      (uint8_t *)CTX_ZCASH_SHIELDED_OUTPUTS_HASH_PERSONALIZATION, 16);
...
void shielded_spend_hash(const uint8_t *input, uint16_t inputlen,
  ...
  cx_blake2b_init2_no_throw(
      &ctx, 256, NULL, 0,
      (uint8_t *)CTX_ZCASH_SHIELDED_SPENDS_HASH_PERSONALIZATION, 16);
  cx_hash_no_throw(&ctx.header, CX_LAST, input, inputlen, output, HASH_SIZE);
}

static void signature_hash_v4(const uint8_t *input, uint16_t inputlen,
                              uint8_t *output) {
...
static void signature_hash_v5(const uint8_t *input, uint8_t *start_signdata,
  ...
  transparent_sig_digest(input, start_signdata, index, type,
                         transparent_digest);
  ...
  cx_hash_no_throw(&ctx.header, CX_LAST, orchard_digest, HASH_SIZE, output,
                   HASH_SIZE);
}
...
static void signature_script_hash_v4(const uint8_t *input, uint16_t inputlen,
                                     uint8_t *script, uint16_t scriptlen,
                                     uint8_t *output) {
  cx_blake2b_t ctx;
...
void signature_script_hash(const uint8_t *input, uint8_t *start_signdata,
  ...
  if (tx_version == TX_VERSION_SAPLING) {
    signature_script_hash_v4(start_signdata, inputlen, script, scriptlen,
                             output);
  } else if (tx_version == TX_VERSION_NU5) {
