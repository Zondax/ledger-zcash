...
void nu5_transparent_sequence_hash(const uint8_t *input, uint8_t *output) {
  ...
  cx_hash_no_throw(&ctx.header, CX_LAST, data, SEQUENCE_SIZE, output,
                   HASH_SIZE);
}
...
void nu5_hash_sapling_spends(const uint8_t *input, uint8_t *output) {
  ...
  MEMCPY(ch_personalization,
         PIC(ZCASH_SAPLING_SPENDS_COMPACT_HASH_PERSONALIZATION), 16);
  cx_blake2b_init2_no_throw(&ch_ctx, 256, NULL, 0,
                            (uint8_t *)ch_personalization, 16);

  cx_blake2b_t nh_ctx;
  uint8_t nh_personalization[16] = {0};
  MEMCPY(nh_personalization,
         PIC(ZCASH_SAPLING_SPENDS_NONCOMPACT_HASH_PERSONALIZATION), 16);
  cx_blake2b_init2_no_throw(&nh_ctx, 256, NULL, 0,
                            (uint8_t *)nh_personalization, 16);
  ...
  for (uint8_t i = 0; i < n - 1; i++, nullifier_data += SPEND_TX_LEN,
               cv_data += SPEND_TX_LEN, anchor_data += SPEND_TX_LEN,
               rk_data += SPEND_TX_LEN) {
    // build the hash of nullifiers separately for compact blocks.
    cx_hash_no_throw(&ch_ctx.header, 0, nullifier_data, NULLIFIER_SIZE, NULL,
                     0);

    cx_hash_no_throw(&nh_ctx.header, 0, cv_data, VALUE_COMMITMENT_SIZE, NULL,
                     0);
  ...
  cx_hash_no_throw(&ch_ctx.header, CX_LAST, nullifier_data, NULLIFIER_SIZE,
                   (uint8_t *)ch_out, HASH_SIZE);

  uint8_t nh_out[HASH_SIZE] = {0};
  cx_hash_no_throw(&nh_ctx.header, 0, cv_data, VALUE_COMMITMENT_SIZE, NULL, 0);
  cx_hash_no_throw(&nh_ctx.header, 0, anchor_data, ANCHOR_SIZE, NULL, 0);
  cx_hash_no_throw(&nh_ctx.header, CX_LAST, rk_data, RK_SIZE, (uint8_t *)nh_out,
                   HASH_SIZE);

  cx_hash_no_throw(&ctx.header, 0, (uint8_t *)ch_out, HASH_SIZE, NULL, 0);
  cx_hash_no_throw(&ctx.header, CX_LAST, (uint8_t *)nh_out, HASH_SIZE, output,
                   HASH_SIZE);
}
...
void nu5_hash_sapling_outputs(const uint8_t *input, uint8_t *output) {
  ...
  uint8_t ch_personalization[16] = {0};
  MEMCPY(ch_personalization,
         PIC(ZCASH_SAPLING_OUTPUTS_COMPACT_HASH_PERSONALIZATION), 16);
  cx_blake2b_init2_no_throw(&ch_ctx, 256, NULL, 0,
                            (uint8_t *)ch_personalization, 16);

  cx_blake2b_t mh_ctx;
  uint8_t mh_personalization[16] = {0};
  MEMCPY(mh_personalization,
         PIC(ZCASH_SAPLING_OUTPUTS_MEMOS_HASH_PERSONALIZATION), 16);
  cx_blake2b_init2_no_throw(&mh_ctx, 256, NULL, 0,
                            (uint8_t *)mh_personalization, 16);

  cx_blake2b_t nh_ctx;
  uint8_t nh_personalization[16] = {0};
  MEMCPY(nh_personalization,
         PIC(ZCASH_SAPLING_OUTPUTS_NONCOMPACT_HASH_PERSONALIZATION), 16);
  cx_blake2b_init2_no_throw(&nh_ctx, 256, NULL, 0,
                            (uint8_t *)nh_personalization, 16);
  ...
  for (uint8_t i = 0; i < n - 1; i++, cmu += OUTPUT_TX_LEN,
               ephemeral_key += OUTPUT_TX_LEN, enc_ciphertext += OUTPUT_TX_LEN,
               enc_ciphertext_memo += OUTPUT_TX_LEN, cv += OUTPUT_TX_LEN,
    ...
    cx_hash_no_throw(&mh_ctx.header, 0, enc_ciphertext_memo,
                     OUTPUT_ENC_MEMO_SIZE, NULL, 0);

    cx_hash_no_throw(&nh_ctx.header, 0, cv, VALUE_COMMITMENT_SIZE, NULL, 0);
    cx_hash_no_throw(&nh_ctx.header, 0, enc_ciphertext_aead_tag,
                     OUTPUT_ENC_AEAD_TAG_SIZE, NULL, 0);
    cx_hash_no_throw(&nh_ctx.header, 0, out_ciphertext, OUTPUT_OUT_SIZE, NULL,
                     0);
  }
  ...
  cx_hash_no_throw(&ch_ctx.header, 0, cmu, NOTE_COMMITMENT_SIZE, NULL, 0);
  cx_hash_no_throw(&ch_ctx.header, 0, ephemeral_key, EPK_SIZE, NULL, 0);
  cx_hash_no_throw(&ch_ctx.header, CX_LAST, enc_ciphertext, 52, ch_out,
                   HASH_SIZE);

  uint8_t mh_out[HASH_SIZE] = {0};
  cx_hash_no_throw(&mh_ctx.header, CX_LAST, enc_ciphertext_memo,
                   OUTPUT_ENC_MEMO_SIZE, (uint8_t *)mh_out, HASH_SIZE);

  uint8_t nh_out[HASH_SIZE] = {0};
  cx_hash_no_throw(&nh_ctx.header, 0, cv, VALUE_COMMITMENT_SIZE, NULL, 0);
  cx_hash_no_throw(&nh_ctx.header, 0, enc_ciphertext_aead_tag,
                   OUTPUT_ENC_AEAD_TAG_SIZE, NULL, 0);
  cx_hash_no_throw(&nh_ctx.header, CX_LAST, out_ciphertext, OUTPUT_OUT_SIZE,
                   nh_out, HASH_SIZE);
...
void hash_header_txid_data(const uint8_t *input, uint8_t *output) {
  ...
  const uint8_t *consensus_branch_id =
      input + NU5_INDEX_HASH_CONSENSUS_BRANCH_ID;
...
void hash_transparent_txid_data(const uint8_t *input, uint8_t *output) {
  ...
  if (t_inlist_len() + t_outlist_len() > 0) {
    ...
    cx_hash_no_throw(&ctx.header, CX_LAST, outputs_digest, HASH_SIZE, output,
                     HASH_SIZE);
  } else {
...
void transparent_sig_digest(const uint8_t *input, uint8_t *start_signdata,
  ...
  if (t_inlist_len() == 0) {
    ...
    // the following implies that flag_anyonecanpay = flag_single = flag_none =
    // false
    uint8_t hash_type = SIGHASH_ALL;

    const uint8_t *prevout_digest =
        start_signdata + NU5_INDEX_HASH_PREVOUTSHASH;

    // compute amounts digest
    cx_blake2b_t ctx_amounts;
    uint8_t personalization[16] = {0};
    MEMCPY(personalization, PIC(ZCASH_TRANSPARENT_AMOUNTS_HASH_PERSONALIZATION),
           16);
    cx_blake2b_init2_no_throw(&ctx_amounts, 256, NULL, 0,
    ...
    for (uint8_t i = 0; i < t_inlist_len() - 1; ++i) {
      ...
      cx_hash_no_throw(&ctx_amounts.header, 0, (uint8_t *)&amount,
                       sizeof(uint64_t), NULL, 0);
    }
    amount = t_inlist_retrieve_item_amount(t_inlist_len() - 1);
    cx_hash_no_throw(&ctx_amounts.header, CX_LAST, (uint8_t *)&amount,
                     sizeof(uint64_t), amounts_digest, HASH_SIZE);
    MEMZERO(personalization, 16);

    cx_blake2b_t ctx_scripts;
    MEMCPY(personalization, PIC(ZCASH_TRANSPARENT_SCRIPTS_HASH_PERSONALIZATION),
           16);
    cx_blake2b_init2_no_throw(&ctx_scripts, 256, NULL, 0,
                              (uint8_t *)personalization, 16);
    uint8_t scripts[SCRIPT_SIZE];
    uint8_t scripts_digest[HASH_SIZE] = {0};
    for (uint8_t i = 0; i < t_inlist_len() - 1; ++i) {
      t_inlist_retrieve_item_script(i, scripts);
      cx_hash_no_throw(&ctx_scripts.header, 0, scripts, sizeof(scripts), NULL,
                       0);
      MEMZERO(scripts, SCRIPT_SIZE);
    }
    t_inlist_retrieve_item_script(t_inlist_len() - 1, scripts);
    cx_hash_no_throw(&ctx_scripts.header, CX_LAST, scripts, SCRIPT_SIZE,
                     scripts_digest, HASH_SIZE);
    MEMZERO(personalization, 16);

    const uint8_t *sequence_digest =
        start_signdata + NU5_INDEX_HASH_SEQUENCEHASH;
    ...
    MEMCPY(personalization, PIC(ZCASH_TRANSPARENT_INPUT_HASH_PERSONALIZATION),
           16);
    cx_blake2b_init2_no_throw(&ctx_txin_sig_digest, 256, NULL, 0,
                              (uint8_t *)personalization, 16);
    if (type == transparent) {
      const t_input_item_t *item = t_inlist_retrieve_item(index);

      const uint8_t *prevout_data =
          input + index * T_IN_TX_LEN + INDEX_TIN_PREVOUT;
      cx_hash_no_throw(&ctx_txin_sig_digest.header, 0, prevout_data,
                       PREVOUT_SIZE, NULL, 0);

      uint64_t value = item->value;
      cx_hash_no_throw(&ctx_txin_sig_digest.header, 0, (uint8_t *)&value,
                       sizeof(uint64_t), NULL, 0);

      const uint8_t *script = item->script;
      cx_hash_no_throw(&ctx_txin_sig_digest.header, 0, script, SCRIPT_SIZE,
                       NULL, 0);

      const uint8_t *sequence_data =
          input + index * T_IN_TX_LEN + INDEX_TIN_SEQ;
      cx_hash_no_throw(&ctx_txin_sig_digest.header, 0, sequence_data,
                       SEQUENCE_SIZE, NULL, 0);
    }
    cx_hash_no_throw(&ctx_txin_sig_digest.header, CX_LAST, NULL, 0,
                     txin_sig_digest, HASH_SIZE);
    MEMZERO(personalization, 16);

    cx_blake2b_t ctx;
    MEMCPY(personalization, PIC(ZCASH_TRANSPARENT_HASH_PERSONALIZATION), 16);
    cx_blake2b_init2_no_throw(&ctx, 256, NULL, 0, (uint8_t *)personalization,
                              16);
    ...
    cx_hash_no_throw(&ctx.header, CX_LAST, txin_sig_digest, HASH_SIZE, output,
                     HASH_SIZE);
  }
}
...
void hash_sapling_txid_data(const uint8_t *input, uint8_t *output) {
  ...
  if (spendlist_len() + outputlist_len() > 0) {
    const uint8_t *hash_sapling_spends =
        input + NU5_INDEX_HASH_SHIELDEDSPENDHASH;
    const uint8_t *hash_sapling_outputs =
        input + NU5_INDEX_HASH_SHIELDEDOUTPUTHASH;
    ...
    cx_hash_no_throw(&ctx.header, CX_LAST, value_balance, NU5_VALUEBALANCE_SIZE,
                     output, HASH_SIZE);
  } else {
    cx_hash_no_throw(&ctx.header, CX_LAST, 0, 0, output, HASH_SIZE);
  }
