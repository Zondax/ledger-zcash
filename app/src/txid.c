...
zxerr_t nu5_hash_sapling_outputs(const uint8_t *input, uint8_t *output) {
  ...
  for (uint8_t i = 0; i < n - 1; i++, cmu += OUTPUT_TX_LEN, ephemeral_key += OUTPUT_TX_LEN,
                                      enc_ciphertext += OUTPUT_TX_LEN, enc_ciphertext_memo += OUTPUT_TX_LEN,
                                      cv += OUTPUT_TX_LEN, enc_ciphertext_aead_tag += OUTPUT_TX_LEN,
  ...
  CHECK_CX_OK(cx_hash_no_throw(&mh_ctx.header, CX_LAST, enc_ciphertext_memo,OUTPUT_ENC_MEMO_SIZE, (uint8_t *)mh_out, HASH_SIZE));
...
zxerr_t transparent_sig_digest(const uint8_t *input, uint8_t *start_signdata,
                              uint8_t index, signable_input type, uint8_t *output) {
