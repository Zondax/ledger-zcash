...
parser_error_t check_swap_conditions(const parser_context_t *ctx) {
    ...
    if ( (t_inlist_len() > 1) || (t_outlist_len()>1)) {
        // swaps can only be a transparent transaction with one t-in and one t-out
    ...
    const t_output_item_t *item = t_outlist_retrieve_item(1);

    err = parser_sapling_display_address_t(item->address, tmp_str, sizeof(tmp_str), 0, &pageCount);
    ...
    err = parser_sapling_display_value(item->value, tmp_str, sizeof(tmp_str), 0, &pageCount);
