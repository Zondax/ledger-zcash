...
parser_error_t parser_validate() {
  ...
  for (uint8_t idx = 0; idx < numItems; idx++) {
    uint8_t pageCount = 0;
    CHECK_PARSER_ERR(parser_getItem(idx, tmpKey, sizeof(tmpKey), tmpVal,
...
parser_error_t parser_sapling_display_address_t(const uint8_t *addr,
                                                char *outVal,
                                                uint16_t outValLen,
                                                uint8_t pageIdx,
                                                uint8_t *pageCount) {
  ...
  cx_hash_sha256(address, VERSION_SIZE + CX_RIPEMD160_SIZE,
  ...
  int err =
      encode_base58(address, VERSION_SIZE + CX_RIPEMD160_SIZE + CHECKSUM_SIZE,
                    tmpBuffer, &outLen);
  if (err != 0) {
...
parser_error_t parser_sapling_display_address_s(uint8_t *div, uint8_t *pkd,
                                                char *outVal,
                                                uint16_t outValLen,
                                                uint8_t pageIdx,
                                                uint8_t *pageCount) {
  ...
  char tmpBuffer[100];
  bech32EncodeFromBytes(tmpBuffer, sizeof(tmpBuffer), BECH32_HRP, address,
                        sizeof(address), 1, BECH32_ENCODING_BECH32);
  pageString(outVal, outValLen, tmpBuffer, pageIdx, pageCount);
  return parser_ok;
}

parser_error_t parser_sapling_getTypes(const uint16_t displayIdx,
  ...
  *pageCount = 1;
  ...
  case type_tin: {
    ...
    case 0: {
      ...
      return parser_sapling_display_address_t(item->script, outVal, outValLen,
                                              pageIdx, pageCount);
    }
    case 1: {
      snprintf(outKey, outKeyLen, "T-in (ZEC)");
      return parser_sapling_display_value(item->value, outVal, outValLen,
                                          pageIdx, pageCount);
    }
    }
  }
  ...
  case type_tout: {
    ...
    case 0: {
      ...
      return parser_sapling_display_address_t(item->address, outVal, outValLen,
                                              pageIdx, pageCount);
    }
    case 1: {
  ...
  case type_sspend: {
    ...
    case 0: {
      ...
      return parser_sapling_display_address_s(item->div, item->pkd, outVal,
  ...
  case type_txfee: {
    ...
    return parser_sapling_display_value(get_totalvalue(), outVal, outValLen,
                                        pageIdx, pageCount);
  }
