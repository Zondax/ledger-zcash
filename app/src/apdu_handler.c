...
__Z_INLINE void handleExtractSpendSignature(volatile uint32_t *tx,
  ...
  *tx = 0;
  ...
  zxerr_t err =
      crypto_extract_spend_signature(G_io_apdu_buffer, IO_APDU_BUFFER_SIZE - 2);
...
__Z_INLINE void handleExtractTransparentSignature(volatile uint32_t *tx,
                                                  uint32_t rx) {
  ...
  *tx = 0;
  ...
  zxerr_t err = crypto_extract_transparent_signature(G_io_apdu_buffer,
...
__Z_INLINE void handleExtractSpendData(volatile uint32_t *tx, uint32_t rx) {
  ...
  *tx = 0;
  ...
  zxerr_t err = crypto_extract_spend_proofkeyandrnd(G_io_apdu_buffer,
                                                    IO_APDU_BUFFER_SIZE - 2);
  view_tx_state();
  if (err == zxerr_ok) {
...
__Z_INLINE void handleExtractOutputData(volatile uint32_t *tx, uint32_t rx) {
  ...
  *tx = 0;
  ...
  uint16_t replyLen = 0;
  zxerr_t err = crypto_extract_output_rnd(G_io_apdu_buffer,
                                          IO_APDU_BUFFER_SIZE - 2, &replyLen);
  view_tx_state();
  if (err == zxerr_ok) {
    *tx = replyLen;
    THROW(APDU_CODE_OK);
...
__Z_INLINE void handleInitTX(volatile uint32_t *flags, volatile uint32_t *tx,
                             uint32_t rx) {
  ...
  *tx = 0;
  ...
  zxerr_t err = crypto_extracttx_sapling(
      G_io_apdu_buffer, IO_APDU_BUFFER_SIZE - 3, message, messageLength);
  if (err != zxerr_ok) {
    transaction_reset();
  ...
  err = crypto_hash_messagebuffer(G_io_apdu_buffer, IO_APDU_BUFFER_SIZE - 3,
...
__Z_INLINE void handleGetKeyIVK(volatile uint32_t *flags, volatile uint32_t *tx,
                                uint32_t rx) {
  ...
  *tx = 0;
  ...
  parser_error_t prserr = parser_sapling_path(G_io_apdu_buffer + OFFSET_DATA,
                                              DATA_LENGTH_GET_IVK, &zip32path);
  MEMZERO(G_io_apdu_buffer, IO_APDU_BUFFER_SIZE);
  if (prserr != parser_ok) {
    *tx = 0;
    THROW(APDU_CODE_DATA_INVALID);
  ...
  zxerr_t err = crypto_ivk_sapling(G_io_apdu_buffer, IO_APDU_BUFFER_SIZE - 2,
                                   zip32path, &replyLen);
  if (err != zxerr_ok) {
    *tx = 0;
    THROW(APDU_CODE_DATA_INVALID);
...
__Z_INLINE void handleGetKeyOVK(volatile uint32_t *flags, volatile uint32_t *tx,
                                uint32_t rx) {
  ...
  *tx = 0;
  ...
  zxerr_t err = crypto_ovk_sapling(G_io_apdu_buffer, IO_APDU_BUFFER_SIZE - 2,
                                   zip32path, &replyLen);
  if (err != zxerr_ok) {
    *tx = 0;
    THROW(APDU_CODE_DATA_INVALID);
...
__Z_INLINE void handleGetKeyFVK(volatile uint32_t *flags, volatile uint32_t *tx,
                                uint32_t rx) {
  ...
  *tx = 0;
  ...
  zxerr_t err = crypto_fvk_sapling(G_io_apdu_buffer, IO_APDU_BUFFER_SIZE - 2,
                                   zip32path, &replyLen);
  if (err != zxerr_ok) {
    *tx = 0;
    THROW(APDU_CODE_DATA_INVALID);
...
__Z_INLINE void handleGetNullifier(volatile uint32_t *flags,
  ...
  *tx = 0;
  ...
  memcpy(cm,
         G_io_apdu_buffer + OFFSET_DATA + ZIP32_PATH_SIZE + NOTE_POSITION_SIZE,
  ...
  // note commitment
  zxerr_t err = crypto_nullifier_sapling(
      G_io_apdu_buffer, IO_APDU_BUFFER_SIZE - 2, notepos, cm, &replyLen);
  if (err != zxerr_ok) {
...
__Z_INLINE void handleCheckandSign(volatile uint32_t *tx, uint32_t rx) {
  ...
  *tx = 0;
  ...
  zxerr_t err = crypto_check_prevouts(G_io_apdu_buffer, IO_APDU_BUFFER_SIZE - 3,
                                      message, txVersion);
  if (err != zxerr_ok) {
    MEMZERO(G_io_apdu_buffer, IO_APDU_BUFFER_SIZE);
    view_idle_show(0, NULL);
  ...
  err = crypto_check_sequence(G_io_apdu_buffer, IO_APDU_BUFFER_SIZE - 3,
                              message, txVersion);
  if (err != zxerr_ok) {
    MEMZERO(G_io_apdu_buffer, IO_APDU_BUFFER_SIZE);
    view_idle_show(0, NULL);
  ...
  err = crypto_check_outputs(G_io_apdu_buffer, IO_APDU_BUFFER_SIZE - 3, message,
                             messageLength, txVersion);
  if (err != zxerr_ok) {
    MEMZERO(G_io_apdu_buffer, IO_APDU_BUFFER_SIZE);
    view_idle_show(0, NULL);
    transaction_reset();
  ...
  err = crypto_check_joinsplits(G_io_apdu_buffer, IO_APDU_BUFFER_SIZE - 3,
                                message, txVersion);
  if (err != zxerr_ok) {
    MEMZERO(G_io_apdu_buffer, IO_APDU_BUFFER_SIZE);
    view_idle_show(0, NULL);
  ...
  err = crypto_check_valuebalance(G_io_apdu_buffer, IO_APDU_BUFFER_SIZE - 3,
                                  message, txVersion);
  if (err != zxerr_ok) {
    MEMZERO(G_io_apdu_buffer, IO_APDU_BUFFER_SIZE);
    view_idle_show(0, NULL);
  ...
  err = crypto_checkspend_sapling(G_io_apdu_buffer, IO_APDU_BUFFER_SIZE - 3,
                                  message, messageLength, txVersion);
  if (err != zxerr_ok) {
    MEMZERO(G_io_apdu_buffer, IO_APDU_BUFFER_SIZE);
    view_idle_show(0, NULL);
    transaction_reset();
  ...
  err = crypto_checkoutput_sapling(G_io_apdu_buffer, IO_APDU_BUFFER_SIZE - 3,
                                   message, messageLength, txVersion);
  if (err != zxerr_ok) {
  ...
  err = crypto_checkencryptions_sapling(G_io_apdu_buffer,
                                        IO_APDU_BUFFER_SIZE - 3, message);
  if (err != zxerr_ok) {
    MEMZERO(G_io_apdu_buffer, IO_APDU_BUFFER_SIZE);
    view_idle_show(0, NULL);
    transaction_reset();
  ...
  err = crypto_sign_and_check_transparent(G_io_apdu_buffer,
                                          IO_APDU_BUFFER_SIZE - 3, message,
                                          messageLength, txVersion);
  if (err != zxerr_ok) {
    MEMZERO(G_io_apdu_buffer, IO_APDU_BUFFER_SIZE);
  ...
  err = crypto_signspends_sapling(G_io_apdu_buffer, IO_APDU_BUFFER_SIZE - 3,
                                  message, messageLength, txVersion);
  if (err != zxerr_ok) {
    MEMZERO(G_io_apdu_buffer, IO_APDU_BUFFER_SIZE);
    view_idle_show(0, NULL);
...
__Z_INLINE void handleGetAddrSecp256K1(volatile uint32_t *flags,
  ...
  *tx = 0;
  ...
  zxerr_t err = crypto_fillAddress_secp256k1(
      G_io_apdu_buffer, IO_APDU_BUFFER_SIZE - 2, &replyLen);
  if (err != zxerr_ok) {
    *tx = 0;
    THROW(APDU_CODE_DATA_INVALID);
  }
...
__Z_INLINE void handleGetDiversifierList(volatile uint32_t *tx, uint32_t rx) {
  ...
  *tx = 0;
  ...
  parser_error_t prserr = parser_sapling_path_with_div(
      G_io_apdu_buffer + OFFSET_DATA, DATA_LENGTH_GET_DIV_LIST, &parser_addr);
  MEMZERO(G_io_apdu_buffer, IO_APDU_BUFFER_SIZE);
  if (prserr != parser_ok) {
    *tx = 0;
    THROW(APDU_CODE_DATA_INVALID);
  }
  zxerr_t err = crypto_diversifier_with_startindex(
      G_io_apdu_buffer, parser_addr.path, parser_addr.div, &replyLen);
...
__Z_INLINE void handleGetAddrSapling(volatile uint32_t *flags,
  ...
  *tx = 0;
  ...
  uint16_t replyLen = 0;
  zxerr_t err = crypto_fillAddress_sapling(
      G_io_apdu_buffer, IO_APDU_BUFFER_SIZE - 2, zip32path, &replyLen);
  if (err != zxerr_ok) {
    *tx = 0;
    THROW(APDU_CODE_DATA_INVALID);
  }
