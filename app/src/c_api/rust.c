...
zxerr_t c_blake2b32_withpersonal(const uint8_t *person, const uint8_t *a, uint32_t a_len, uint8_t *out) {
    if (person == NULL || a == NULL || out == NULL) {
        return zxerr_no_data;
    }

    cx_blake2b_t ctx = {0};
    CHECK_CX_OK(cx_blake2b_init2_no_throw(&ctx, 256, /* out_len */
...
zxerr_t c_blake2b64_withpersonal(const uint8_t *person, const uint8_t *a, uint32_t a_len, uint8_t *out) {
    if (person == NULL || a == NULL || out == NULL) {
        return zxerr_no_data;
    }

    cx_blake2b_t ctx = {0};
    CHECK_CX_OK(cx_blake2b_init2_no_throw(&ctx, 512, /* out_len */
                                          NULL, 0,   /* salt, salt_len */
                                          (uint8_t *)person, 16));
...
zxerr_t c_zcash_blake2b_redjubjub(const uint8_t *a, uint32_t a_len, const uint8_t *b, uint32_t b_len, uint8_t *out) {
    if (a == NULL || b == NULL || out == NULL) {
        return zxerr_no_data;
    }

    cx_blake2b_t ctx = {0};
    CHECK_CX_OK(cx_blake2b_init2_no_throw(&ctx, 8 * CTX_REDJUBJUB_HASH_LEN, /* out_len */
                                          NULL, 0, (uint8_t *)CTX_REDJUBJUB, CTX_REDJUBJUB_LEN));
...
zxerr_t c_zcash_blake2b_expand_seed(const uint8_t *a, uint32_t a_len, const uint8_t *b, uint32_t b_len, uint8_t *out) {
    if (a == NULL || b == NULL || out == NULL) {
        return zxerr_no_data;
    }

    cx_blake2b_t ctx = {0};
    CHECK_CX_OK(cx_blake2b_init2_no_throw(&ctx, 8 * CTX_EXPAND_SEED_HASH_LEN, NULL, 0, (uint8_t *)CTX_EXPAND_SEED,
                                          CTX_EXPAND_SEED_LEN));
    CHECK_CX_OK(cx_hash_no_throw(&ctx.header, 0, a, a_len, NULL, 0));
...
void c_jubjub_spending_base_scalarmult(uint8_t *point, const uint8_t *scalar) {
    ...
    if (jubjub_extendedpoint_scalarmult(&p, scal) != zxerr_ok || jubjub_extendedpoint_tobytes(point, &p) != zxerr_ok) {
        MEMZERO(point, JUBJUB_FIELD_BYTES);
    }
}
