...
zxerr_t t_inlist_append_item(uint32_t *p, uint8_t *script, uint64_t v) {
  ...
  MEMCPY_NV((void *)&N_t_inlist.items[transaction_header.t_in_len], &newitem,
            sizeof(t_input_item_t));
...
zxerr_t t_outlist_append_item(uint8_t *addr, uint64_t v) {
  ...
  MEMCPY_NV((void *)&N_t_outlist.items[transaction_header.t_out_len], &newitem,
            sizeof(t_output_item_t));
...
uint8_t t_inlist_len() { return transaction_header.t_in_len; }
...
uint8_t t_outlist_len() { return transaction_header.t_out_len; }
...
zxerr_t transparent_signatures_append(uint8_t *signature) {
  ...
  MEMCPY_NV((void *)&N_transactioninfo
                .transparent_signatures[transaction_header.t_sign_index],
...
bool spendlist_is_active() { return transaction_header.spendlist_len > 0; }

zxerr_t spendlist_append_item(uint32_t p, uint64_t v, uint8_t *div,
                              uint8_t *pkd, uint8_t *rcm, uint8_t *alpha) {
...
spend_item_t *spendlist_extract_next() {
  if (transaction_header.spendlist_len <=
      transaction_header.spenddata_extract_index) {
    return NULL;
  } else {
    spend_item_t *result =
        (spend_item_t *)&N_spendlist
            .items[transaction_header.spenddata_extract_index];
...
uint8_t spendlist_len() { return transaction_header.spendlist_len; }

bool outputlist_is_active() { return transaction_header.outputlist_len > 0; }

zxerr_t outputlist_append_item(uint8_t *d, uint8_t *pkd, uint64_t v,
                               uint8_t memotype, uint8_t *ovk, uint8_t *rcmv,
                               uint8_t *rseed) {
...
output_item_t *outputlist_extract_next() {
  if (transaction_header.outputlist_len <=
      transaction_header.outputdata_extract_index) {
    return NULL;
  } else {
    output_item_t *result =
        (output_item_t *)&N_outputlist
            .items[transaction_header.outputdata_extract_index];
...
uint8_t outputlist_len() { return transaction_header.outputlist_len; }
...
int64_t get_valuebalance() { return transaction_header.sapling_value; }
uint64_t get_totalvalue() { return transaction_header.total_value; }
uint8_t get_state() { return transaction_header.state; }

void set_state(uint8_t state) { transaction_header.state = state; }

void state_reset() { transaction_header.state = STATE_INITIAL; }
