...
describe('End to end transactions', function () {
  ...
  test.concurrent.each(models)('make a tx with 1 transparent input 1 spend 2 shielded outputs', async function (m) {
    const sim = new Zemu(m.path)
    try {
      await sim.start({ ...defaultOptions, model: m.name })
      const app = new ZCashApp(sim.getTransport())
  ...
  test.concurrent.each(models)('make a tx with 1 transparent output 1 spend 2 shielded outputs', async function (m) {
    const sim = new Zemu(m.path)
    try {
      await sim.start({ ...defaultOptions, model: m.name })
      const app = new ZCashApp(sim.getTransport())
  ...
  test.concurrent.each(models)(
    'make a transaction with 1 transparent input 1 transparent output 1 spend 2 shielded outputs',
    async function (m) {
      const sim = new Zemu(m.path)
      try {
        await sim.start({ ...defaultOptions, model: m.name })
  ...
  test.concurrent.each(models)('make a transaction with 2 transparent input 2 transparent output', async function (m) {
    const sim = new Zemu(m.path)
    try {
      await sim.start({ ...defaultOptions, model: m.name })
      const app = new ZCashApp(sim.getTransport())
  ...
  test.concurrent.each(models)('extracting signatures without checkandsign', async function (m) {
    const sim = new Zemu(m.path)
    try {
      await sim.start({ ...defaultOptions, model: m.name })
      const app = new ZCashApp(sim.getTransport())
...
describe('Failing transactions', function () {
  test.concurrent.each(models)('try to extract spend data without calling inittx', async function (m) {
    const sim = new Zemu(m.path)
    try {
      await sim.start({ ...defaultOptions, model: m.name })
      const app = new ZCashApp(sim.getTransport())
  ...
  test.concurrent.each(models)('extracting output without extracting spend data', async function (m) {
    const sim = new Zemu(m.path)
    try {
      await sim.start({ ...defaultOptions, model: m.name })
      const app = new ZCashApp(sim.getTransport())
  ...
  test.concurrent.each(models)('extracting more signatures than needed for tx', async function (m) {
    const sim = new Zemu(m.path)
    try {
      await sim.start({ ...defaultOptions, model: m.name })
      const app = new ZCashApp(sim.getTransport())
  ...
  test.concurrent.each(models)('not using ledger randomness for tx', async function (m) {
    const sim = new Zemu(m.path)
    try {
      await sim.start({ ...defaultOptions, model: m.name })
      const app = new ZCashApp(sim.getTransport())
  ...
  test.concurrent.each(models)('use other address in builder than in inittx', async function (m) {
    const sim = new Zemu(m.path)
    try {
      await sim.start({ ...defaultOptions, model: m.name })
      const app = new ZCashApp(sim.getTransport())
  ...
  test.concurrent.each(models)('try txfee of 10000', async function (m) {
    const sim = new Zemu(m.path)
    try {
      await sim.start({ ...defaultOptions, model: m.name })
      const app = new ZCashApp(sim.getTransport())
  ...
  test.concurrent.each(models)('extract data after tx reject', async function (m) {
    const sim = new Zemu(m.path)
    try {
  ...
  test.concurrent.each(models)('make a transaction unsupported transaction version', async function (m) {
    const sim = new Zemu(m.path)
